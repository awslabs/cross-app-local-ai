...
public final class KokoroPhonemizer {
    ...
    private var g2pBosId = 1
    private var g2pEosId = 2
    private var g2pPadId = 0

    /// NL tagger for POS tagging (heteronym resolution).
    private let tagger = NLTagger(tagSchemes: [.lexicalClass])

    /// Look up the token ID for a given IPA symbol.
    ///
    /// - Parameter symbol: An IPA symbol string (e.g. " " for space).
    /// - Returns: The token ID, or `nil` if the symbol is not in the vocabulary.
    func tokenId(for symbol: String) -> Int? {
        vocabMap[symbol]
    }
    ...
    public func loadG2PModels(encoderURL: URL, decoderURL: URL, vocabURL: URL) throws {
        ...
        encoderConfig.modelDisplayName = "kokoro-g2p-encoder"

        let decoderConfig = MLModelConfiguration()
        decoderConfig.computeUnits = .cpuOnly
        decoderConfig.modelDisplayName = "kokoro-g2p-decoder"

        g2pEncoder = try MLModel(contentsOf: encoderURL, configuration: encoderConfig)
        g2pDecoder = try MLModel(contentsOf: decoderURL, configuration: decoderConfig)
    ...
    public func loadG2PModel(from url: URL) throws {
        ...
        config.modelDisplayName = "kokoro-g2p-encoder"
    ...
    public func tokenize(_ text: String, maxLength: Int = 510, language: String = "en") -> [Int] {
        let phonemes: String = switch language {
        case "zh", "cmn", "chinese", "mandarin":
            chinesePhonemizer.phonemize(text)
        case "ja", "japanese":
            japanesePhonemizer.phonemize(text)
        case "it", "italian":
            phonemizeWithDict(text, dict: PronunciationDicts.it, fallback: italianPhonemizer)
        case "fr", "french":
            phonemizeWithDict(text, dict: PronunciationDicts.fr, fallback: frenchPhonemizer)
        case "es", "spanish":
            phonemizeWithDict(text, dict: PronunciationDicts.es, fallback: spanishPhonemizer)
        case "pt", "portuguese":
            phonemizeWithDict(text, dict: PronunciationDicts.pt, fallback: portuguesePhonemizer)
        case "hi", "hindi":
            phonemizeWithDict(text, dict: PronunciationDicts.hi, fallback: hindiPhonemizer)
        default:
            textToPhonemes(text)
        }
    ...
    private func resolveEntry(_ entry: DictEntry, pos: String?) -> String {
        switch entry {
        case let .simple(phonemes):
            return phonemes
        case let .heteronym(posMap):
