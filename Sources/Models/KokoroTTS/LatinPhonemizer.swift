...
final class LatinPhonemizer {
    ...
    func phonemizeWord(_ word: String) -> String {
        convertWord(word.lowercased())
    }

    private func convertWord(_ word: String) -> String {
        var ipa: String = switch language {
        case .french: frenchToIPA(word)
        case .spanish: spanishToIPA(word)
        case .portuguese: portugueseToIPA(word)
        case .italian: italianToIPA(word)
        }
    ...
    private func frenchToIPA(_ word: String) -> String {
        ...
        while i < chars.count {
            ...
            if i + 1 < chars.count {
                ...
                let nasalFollowedByVowelOrNM = afterNasal != nil && "aeiouyéèêëàâùûîïôüœ"
                    .contains(afterNasal!) || afterNasal == "n" || afterNasal == "m"
                if !nasalFollowedByVowelOrNM {
                    switch pair {
                    case "on", "om": result += "ɔ̃"
                        i += 2
                        continue
                    case "an", "am": result += "ɑ̃"
                        i += 2
                        continue
                    case "en", "em": result += "ɑ̃"
                        i += 2
                        continue
                    case "in", "im": result += "ɛ̃"
                        i += 2
                        continue
                    case "un", "um": result += "œ̃"
                        i += 2
                        continue
                    default: break
                    }
